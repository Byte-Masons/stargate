...
contract Router is IStargateRouter, Ownable, ReentrancyGuard {
    ...
    function redeemLocal(
        ...
        bridge.redeemLocal{value: msg.value}(
            _dstChainId,
            _srcPoolId,
            _dstPoolId,
            _refundAddress,
    ...
    function retryRevert(
        ...
        // for retrying the swapRemote. if it fails again, retry
            ...
            (
                ...
                Pool.SwapObj memory s,
                bytes memory p
            ) = abi.decode(payload, (uint8, uint256, uint256, uint256, address, Pool.SwapObj, bytes));
    ...
    function redeemLocalCheckOnRemote(
        ...
        try pool.redeemLocalCheckOnRemote(_srcChainId, _srcPoolId, _amountSD) returns (
            uint256 redeemAmountSD,
            uint256 mintAmountSD
        ) {
    ...
    function _swapRemote(
        ...
        try pool.swapRemote(_srcChainId, _srcPoolId, _to, _s) returns (uint256 amountLD) {
            if (_payload.length > 0) {
                ...
                try
                    IStargateReceiver(_to).sgReceive{gas: _dstGasForCall}(
                        _srcChainId,
                        _srcAddress,
                        _nonce,
                        pool.token(),
